    ...
    var app = $.sammy('#main',function(){
        ...
        this.get('#/', function(){
            this.partial('templates/login.hb', function(){
				var regexIpAdress= /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/
				var regexDigit = new RegExp('^[0-9]+$');
				...
				var user = $("#username").val();
									...
									var portAdd = $("#port").val()
									if(user == ""){
											$("#loginInfo").text("Username is required");
											loginValidate = false
									}
									else if(pwd == ""){
											$("#loginInfo").text("Password is required");
											loginValidate = false
									}
									else if(hostName == ""){
											$("#loginInfo").text("Host is required");
											loginValidate = false
									}
									else if(portAdd == ""){
											$("#loginInfo").text("Port is required");
											loginValidate = false
									}
									else if(!regexIpAdress.test(hostName)){
											$("#loginInfo").text("Invalid host");
											loginValidate = false
									}
									else if(!regexDigit.test(portAdd)){
											$("#loginInfo").text("Invalid port address");
											loginValidate = false
									}
									else
									{
        ...
        this.post('#/login', function(){
			...
			if(loginValidate == true){
					...
					AppWarp.WarpClient.Admin.ValidateCredentials(this.params.username, this.params.password, this.params.host, this.params.port, function(res){
						...
						$("#loginInfo").text("Please make sure your server is up and running,And you have entered correct port and host");
						$("#btnWrapper").html('<input type="submit" class="button" value="Sign in" id="signInBtn">');
						$("#signInBtn").click(function(){
							$("#btnWrapper").html('<input type="button" class="buttonD" value="Signing In...">');
							$("#loginForm").submit();
        ...
        this.get('#/dashboard', function(){
            if(params != null)
                this.partial('templates/dashboard.hb', function(){
                    ...
                    var memchart = new Highcharts.Chart({
                        title: {
                            text: 'Memory',
                            x: -20 //center
                        },
                    ...
                    var roomschart = new Highcharts.Chart({
                        title: {
                            text: 'Rooms',
                            x: -20 //center
                        },
                    ...
                    var ccuchart = new Highcharts.Chart({
                        title: {
                            text: 'CCU',
                            x: -20 //center
                        },
                    ...
                    AppWarp.WarpClient.Admin.GetLiveStats(params.username, params.password, params.host, params.port, function(res){
                        if(res.getResultCode() == AppWarp.ResultCode.Success){
                            ...
                            var memoryseries = memchart.get("memId"); //get series by id
                            memoryseries.setData(memory);
                            var roomseries = roomschart.get("roomId"); //get series by id
                            roomseries.setData(rooms);
                            var ccuseries = ccuchart.get("ccuId"); //get series by id
                            ccuseries.setData(ccu);
                        }
                        else{
