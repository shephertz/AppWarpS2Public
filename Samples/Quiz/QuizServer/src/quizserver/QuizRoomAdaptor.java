...
public class QuizRoomAdaptor extends BaseRoomAdaptor {
	...
	private ArrayList<Integer> answerReceivedList=new ArrayList<Integer>();
    
    public QuizRoomAdaptor(IZone izone, IRoom room) {
        this.izone = izone;
        this.gameRoom = room;
        GAME_STATUS = QuizConstants.STOPPED;
        answerReceivedList.clear();
        try {
    ...
    public void handleUserJoinRequest(IUser user, HandlingResult result) {
        System.out.println(user.getName() + " joined room Request"+ gameRoom.getMaxUsers());
        ...
        if(UserStatusList.size()>1){
        	String username = UserStatusList.get(0).getUser().getName();
        	user.SendChatNotification("server", username, gameRoom);	
        }
                    ...
                    for (int i = 0; i < UserStatusList.size(); i++) {
                        if (UserStatusList.get(i).getUser().getName().equalsIgnoreCase(sender.getName())) {
                            ...
                            if (UserStatusList.get(i).getAnswersPerLevel().size() == GameCurrentLevel) {
                                ...
                                answerReceivedList.add(i);
                                System.out.println("AnswerReceived"+ i);
                            }
    ...
    public void onTimerTick(long time) {
       ...
       try {
        ...
        if(gameRoom!=null && gameRoom.getJoinedUsers()!=null)
        { 
            ...
            GameCurrentLevel=GameCurrentLevel%5;
            ...
            QuizTimerCount++;
        //   System.out.println("QUIZ QuizTimerCount : "+QuizTimerCount);
                if (GameCurrentLevel < Utils.LevelJson.length()) {
                   ...
                   // System.out.println("QUIZ TimeCount : "+timeCount);
                    if ( QuizTimerCount == timeCount || (gameRoom.getMaxUsers()==1 && answerReceivedList.size()==1) ||(gameRoom.getMaxUsers()==2 && answerReceivedList.size()==2)) {
                    
                        int totalQuestion = Utils.LevelJson.getJSONObject(GameCurrentLevel).getInt("totalQuestions");
                       // System.out.println("QUIZ Game Current Level "+ GameCurrentLevel+ " Total Questions "+totalQuestion+" Game Current Question "+GameCurrentQuestion);
    ...
    private void AddDefaultAnswers() {
         ...
         */
    	System.out.println("Add Default Called");
        for (int i = 0; i < UserStatusList.size(); i++) {
            if (UserStatusList.get(i).getAnswersPerLevel().size() == GameCurrentLevel) {
            	System.out.println("Add Default Called Condition matched");    
            	UserStatusList.get(i).getAnswersPerLevel().add(new ArrayList<QuizAnswer>());
            	 int currentSize = UserStatusList.get(i).getAnswersPerLevel().get(GameCurrentLevel).size();
